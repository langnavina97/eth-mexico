...
async function main() {
  ...
  const myModule = await MyModule.deploy(newSafeAddress);
  console.log("MyModule deployed to: ", myModule.address);

  // await run("verify:verify", {
  //   address: myModule.address,
  //   constructorArguments: [newSafeAddress],
  //   contract: "contracts/vault/MyModule.sol:MyModule"
  // });

  let ABI = ["function enableModule(address module)"];
  let abiEncode = new ethers.utils.Interface(ABI);
  let txData = abiEncode.encodeFunctionData("enableModule", [myModule.address]);
